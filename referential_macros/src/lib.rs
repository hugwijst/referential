...
use quote::{quote};
use syn::{parse::Parse, Error, GenericArgument, punctuated::Punctuated, token::Comma};
...
fn get_ty_generics(ty: &syn::Type) -> Vec<GenericArgument> {
    match ty {
        ...
        syn::Type::Reference(ty) => {
            let mut params = ty.lifetime.as_ref().map_or_else(Vec::new, |lt| vec![GenericArgument::Lifetime(lt.clone())]);
            params.extend(get_ty_generics(&ty.elem));
            params
        },
        syn::Type::Slice(ty) => get_ty_generics(&ty.elem),
        syn::Type::TraitObject(_) => panic!("todo"),
        syn::Type::Tuple(ty) => ty.elems.iter().map(|ty| get_ty_generics(ty).into_iter()).flatten().collect(),
...
fn replace_lifetime_ty(ty: &mut syn::Type, orig: &syn::Lifetime, new: &syn::Lifetime) {
    match ty {
        ...
        syn::Type::Reference(ty) => {
            if let Some(lt) = ty.lifetime.as_mut().filter(|lt| lt == &orig) { *lt = new.clone() };
            replace_lifetime_ty(&mut ty.elem, orig, new)
        },
        syn::Type::Slice(ty) => replace_lifetime_ty(&mut ty.elem, orig, new),
        syn::Type::TraitObject(_) => panic!("todo"),
        syn::Type::Tuple(ty) => ty.elems.iter_mut().for_each(|ty| replace_lifetime_ty(ty, orig, new)),
        syn::Type::Verbatim(_) => (),
        _ => (),
    }
}

fn replace_lifetime_generic_param(param: &mut syn::GenericParam, orig: &syn::Lifetime, new: &syn::Lifetime) {
    match param {
        syn::GenericParam::Type(ty) => {
            ...
            ty.default.as_mut().map(|def_ty| replace_lifetime_ty(def_ty, orig, new));
        },
...
fn referential_impl(attr: Attributes, item_struct: syn::ItemStruct) -> syn::Result<TokenStream> {
    ...
    let field = match &item_struct.fields {
        syn::Fields::Named(_) => Err(Error::new_spanned(item_struct.fields, "expected single unnamed field, found named fields")),
        syn::Fields::Unnamed(fs) => if fs.unnamed.len() != 1 {
            Err(Error::new_spanned(fs, "expected single unnamed field, found multiple"))
        } else {
            Ok(fs.unnamed.first().unwrap())
        },
        syn::Fields::Unit => Err(Error::new_spanned(item_struct.fields, "expected single unnamed field, found unit struct")),
    }?;
    ...
    let referential_struct_args = struct_params.iter().map(|param| {
        match param {
            syn::GenericParam::Type(ty) => syn::GenericArgument::Type(
                syn::TypePath {qself: None, path: ty.ident.clone().into()}.into()
            ),
    ...
    replace_lifetime_ty(&mut field_ty_lt_elided, attr_owned_lifetime, &elided_lifetime);
    ...
    let mod_ident = REFERENTIAL_COUNT.with(|count_cell| {
        ...
        syn::Ident::new(&format!("__referential_inner_mod_{}", count), Span::call_site())
    });
